...
.\.venv\Scripts\python.exe -m adapters.client --data-root <existing-data-root> mode set SAFE --command-id <unique-id> --grant-id <grant-id> --task-id <task-id>
...
`UNKNOWN` is rendered as an unresolved fact requiring authoritative reconciliation, never as a failed/retryable commit. `PARTIAL` purge is shown as incomplete and its barrier remains visible. A mode change is an authorized, idempotent Runtime command with an immutable `runtime_mode_events` record; it is not a TraceEvent because the current mode command contract has no trace Run/classification binding. This distinction is intentional and remains a Step 9 audit item.
