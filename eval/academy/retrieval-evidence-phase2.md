...
Before running the runner, the fixed fixture `retrieval-evidence-phase2-fixture.json` was frozen and hashed in the machine result. It contains 18 unique synthetic PUBLIC items, 14 fixed query/ground-truth pairs, and 9 parser probes. Ground truth is authored in the fixture, separate from search output. The runner never uses expected IDs to add a result or pack entry; a search miss stays a miss.
...
One actual read-only Host Search query, `SQLite FTS5 MATCH query syntax`, was run against the public SQLite documentation and recorded through `CodexHostedBridge.record_evidence`. Its URL and excerpt are retained only in the disposable external Academy database, not this repository. The sanitized committed result records one receipt, host `www.sqlite.org`, Evidence ID, T1 integrity verdict, Trace event ID, and `CODEX_HOST_DECLARED`; provider/model/request identity is unavailable. That T1 PASS checks the receipt bytes only, not the documentation’s semantic truth. Conflict, insufficient evidence, and injection-like behavior were qualified with the fixed synthetic cases; no live hostile-content URL was visited.
