...
- The persistent pilot's original E2E stores Host-declared official SQLite FTS5 Evidence. In this continuation, an actual Codex web search for a stale 2020 note returned the official URL and excerpt; the captured Host-declared receipt was passed into an isolated Core test which created a TOOL Run with the query Object as input, persisted the source URL/excerpt as Evidence, verified the exact bytes, traced the Evidence, and closed/inspected the Task. The Run is explicitly an adapter receipt, not platform-native search telemetry; no provider or request ID is claimed.
...
| T10 | PARTIAL | Hosted MODEL/TOOL execution, kind-specific manifests, deterministic Tool/E0/E1/E2 initial choices, hard locality constraints, and Trace/DAG evidence pass in `test_hosted_bridge.py` and `test_deterministic_runtime.py::test_cycle_is_rejected_and_dag_route_budget_and_child_manifests_pass`. Current Runtime has one `scheduled_run_id` per DAG node and no attempt relationship or escalation/fallback state transition. Under the no-architecture-expansion boundary, explicit E0→E1/E2 escalation and post-E2 E1 coordinator resumption cannot be truthfully executed or recorded; this remains Hosted `PARTIAL`, not a Standalone Provider deferral. |
| T11 | PARTIAL | The real Codex Search result and official source `https://www.sqlite.org/fts5.html` were captured and passed to `test_observed_codex_search_tool_run_links_query_evidence_verification_and_trace`, which created a Host-declared SEARCH TOOL Run and linked query Object, source-addressed Evidence, T1 exact-byte Verification, Trace/replay and inspect. However, this was a post-search receipt, not a begin-TOOL → actual search → finish-TOOL lifecycle; actual retrieval time/result reference are not bracketed by the Run. The security test stores an injected, contradictory source and leaves its candidate QUARANTINED/UNKNOWN without changing Grants. T1 axes are `NOT_APPLICABLE / UNKNOWN / INDEPENDENT`, and caller-asserted independence is rejected. Still needed: complete source binding, explicit insufficient-evidence INCONCLUSIVE coverage and true pre/post search lifecycle. Independent Search API remains `DEFERRED / NOT_CONFIGURED`. |
| T12 | PARTIAL | `tests/integration/test_t12_persistent_clone.py::PersistentRootCloneRecoveryTests.test_persistent_root_clone_modes_and_purge_snapshot_replay` starts from a consistent clone of the actual Hosted root and performs synthetic Purge, pre-Purge snapshot restore, RECOVERY, Ledger/barrier replay, integrity/schema checks and validated return to NORMAL. It does not yet combine all requested checks in one sequence: post-replay SAFE/STATELESS enforcement, ordinary inspect denial until recovery completion, complete target/hash/path/index non-resurrection assertions and close/reopen after returning to NORMAL. The live root itself was not purged. |
...
- Baseline full regression after fixture stabilization, v6→v10 migration rehearsal, T9 identifier/state-fact and manifest-index purge, Hosted T11 negative cases and T12 deployment-root clone drill: `.venv\Scripts\python.exe -m unittest discover -s tests -v` — **80 passed, 1 environment-gated search-receipt test skipped** in 38.233s. The skipped test was separately run with actual Codex Search query/URL/excerpt and passed as a post-search receipt; that does not satisfy the requested begin/search/finish lifecycle.
...
- T10 remains PARTIAL for the documented one-Run-per-DAG-node/escalation-attempt limitation; no second Provider or routing engine was introduced.
- T11 remains PARTIAL until the actual search is bracketed by the Run lifecycle and insufficient-evidence/conflict/source-binding behavior is directly observed.
- T12 remains PARTIAL until one persistent-root clone drill includes all post-replay mode/access/identifier checks and a close/reopen after validated NORMAL.
