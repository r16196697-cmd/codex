...
def run(*, fixture_path: Path = FIXTURE, output_path: Path = RESULT,
    ...
    output = {
        ...
        "evidence_test_count": result.testsRun,
        ...
        "expected_fail_closed_count": len(fail_closed),
        "observed_fail_closed_count": fail_closed_pass,
        ...
        "recovery_success_count": sum(row["status"] == "PASS" and "RECOVERY" in row["case_id"] for row in case_results),
        "recovery_inconclusive_count": sum(row["status"] == "INCONCLUSIVE" and "RECOVERY" in row["case_id"] for row in case_results),
...
def main() -> int:
    ...
    print(json.dumps({key: output[key] for key in (
        "fixture_sha256", "case_count", "passed_case_count", "blocker_case_count",
        "inconclusive_case_count", "expected_fail_closed_count", "observed_fail_closed_count",
